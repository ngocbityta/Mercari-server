...
export class PostsService implements IPostQuery, IPostCommand {
	...
	async addPost(token: string, left_video?: string, right_video?: string, course_id?: string, exercise_id?: string, described?: string, device_slave?: string, device_master?: string) {
		...
		const media: string[] = [];
		if (left_video) media.push(left_video);
		if (right_video) media.push(right_video);
	...
	async editPost(token: string, postId: string, described?: string, video_indices?: string, left_video?: string, right_video?: string) {
		...
		if ((hasLeftVideoToDelete || hasRightVideoToDelete) && !hasLeftVideoToAdd && !hasRightVideoToAdd) {
		...
		if (currentMedia.length > 0) {
			for (let i = 0; i < currentMedia.length; i++) {
				...
				// Skip deleted videos
				if (isLeftVideo && hasLeftVideoToDelete) continue;
				if (isRightVideo && hasRightVideoToDelete) continue;
		...
		const updatedPost = await this.prisma.post.update({
			...
			data: {
				...
				media: newMedia,
				leftVideo: hasLeftVideoToAdd ? left_video : (hasLeftVideoToDelete ? null : post.leftVideo),
				rightVideo: hasRightVideoToAdd ? right_video : (hasRightVideoToDelete ? null : post.rightVideo),
			},
			include: {
				owner: true,
			},
